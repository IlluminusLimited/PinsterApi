...
task seed_data: :environment do
  ...
  Parallel.map(500.times, in_threads: 4) do
    SeedHelper::UserHelper.generate
  end

  logger.info { "Creating pins!" }

  Parallel.map(2000.times, in_threads: 4) do
    SeedHelper::PinHelper.generate
  end

  logger.info { "Creating assortments!" }

  Parallel.map(500.times, in_processes: Concurrent.processor_count) do
    SeedHelper::AssortmentHelper.generate
  end

  logger.info { "Creating collections!" }

  Parallel.map(2000.times, in_processes: Concurrent.processor_count) do
